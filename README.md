❗ **NB: Archived because of inactivity** ❗

# AWE_winch_control_heavy_kites
An analytical analysis of how to make an optimal winch controller for kites where mass cannot be neglected. Used for my MSc thesis. The goal is to gain insight into how a kite with mass requires different winch control than a kite without mass, for which the theory is readily available. Optimal control techniques can also be used for this goal, but in this work, a more analytic approach is taken by using the quasi-steady model from [1].
...
- src
  ...
  - `qss_heavy_analysis.ipynb`: Jupyter notebook that investigates the quasi-steady states for an AWE system with mass. It focuses on creating optimal winch control strategies. It uses the `QSSBuilder` class to generate data for many different steady states.
  ...
  - `winch_parameter_analysis.m`: Analyze the effect of different winch parameters.
  - `winch_PI_force_controller.m`: Design of a winch PI force controller.
- workshop: Code from https://github.com/awecourse/workshop, imported as submodule. This code implements the quasi-steady model described in https://doi.org/10.1016/j.renene.2018.07.023.
